...
| **Retrieval Template**| - Defines the query structure using variables like `{CONTEXT}` and `{QUESTION}` to tailor searches to specific needs.<br></br>- Customize templates to closely align with the structure of your data and the nature of the queries, ensuring that retrievals are as relevant as possible.                                               |
