...
**RAG** feature is not supported in Docker mode with `s3fs` yet.
  ...
  <Tabs.Tab>
    ...
    **Installation steps**
    ...
    ```
    - **Step 2**: Verify and modify the configuration options by accessing the `values.yaml` file available at [link](https://github.com/janhq/jan/blob/dev/charts/server/values.yaml) as per your requirements. The following is the example resources created by Jan helm chart:
