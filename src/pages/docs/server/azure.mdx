...
1. Install Docker Engine and Docker Compose using the following command:
<Callout type="info">
...
<Callout type="info">
You can utilize a remote data folder for the autoscaling feature. Refer to the [Advanced Use](/docs/server/azure#advanced-use) section for more details.
</Callout>
<Steps>
### Step 1: Customize the Docker Engine and Compose
1. Set up the docker profile and environment variables.
<Callout type="info">
Please see the [Understanding Jan's Dockerfile](/docs/server/azure#understanding-the-jans-dockerfiles) section below for details on the docker file.
</Callout>
...
### CPU Usage
Running Jan in CPU mode is generally less preferred due to its slower performance than GPU mode. CPUs are optimized for sequential tasks with fewer cores, making them less efficient for parallel processing, which is crucial in AI tasks like neural network training and large dataset handling. With their numerous cores and higher memory bandwidth, GPUs are better suited for these data-intensive tasks and are optimized for AI-specific operations. However, CPU mode can still be used for specific use cases.
