...
<Callout type="info">
You can utilize a Remote Data Folder for the autoscaling feature. Refer to the [Advanced Use](/docs/server/kubernetes#advanced-use) section for more details.
</Callout>
<Steps>
...
1. Get Helm chart from Jan repository by using the following command:
    ...
    ```
2. Verify and modify the configuration options by accessing the `values.yaml` file on `/jan/charts/server`.
