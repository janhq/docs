...
You can also use specific OpenAI models you cannot find in the **Hub** section by customizing the `model.json` file, which you can see in the `~/jan/models/`. Follow the steps explained in the [Using Models](/docs/models#add-a-model-manually) to add a model manually.
