...
### CPU Instruction
Prebuilt Nitro with llama.cpp inference engine needs to run using the correct CPU instruction. AVX2 is the standard CPU instruction set in Jan, commonly used in newer Intel and AMD processors. However, older CPUs that support AVX or AVX-512 require customization of the default CPU instruction.
<Callout type="info">
...
You can enable the GPU acceleration option available within the Jan application. To enable this option, follow the [Hardware Setup](/docs/hardware-setup) guide steps.
## Step-by-step Guide
