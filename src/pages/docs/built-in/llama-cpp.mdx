...
<Tabs items={['Mac', 'Windows', 'Linux']}>
    ...
    <Tabs.Tab >
      ...
      <Callout type="info">
        ...
        - Ensure that you have also installed the correct CPU instruction. Please see [here](/cortex/built-in/llama-cpp) on installing the correct CPU instruction.
      </Callout>
...
Enable the GPU acceleration option available within the Jan application by following the [Installation Setup](/docs/desktop-installation) guide.
## Step-by-step Guide
1. Navigate to the **Settings** > **Advanced Settings**.
2. On the **Jan Data Folder** click the **folder icon (📂)** to access the data folder.
3. Navigate to the `~/jan/engines` folder.
