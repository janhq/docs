...
After installing Jan, a structured data folder is created on the local filesystem. This setup promotes an organized approach to data management. Below is a guide to understanding how files are organized within Jan data folder.
...
### `threads/`
Threads history is kept in this directory. Each session or thread is stored in a way that makes it easy to review past interactions.
...
### Engines
- **Engine Configurations**: Files like `engine_A.json` in the `/engines/` folder configure how Jan can interact with the respective inference engines.

### Extensions
- **Extension Configuration**: The `extensions.json` in the `/extensions/` directory provides settings for all installed extensions.
- **Specific Extensions**: Subdirectories like `/@janhq/extension_A/` contain `package.json` files which import necessary modules for each extension.
...
### Threads
- **Threads History**: Each thread is stored in its subdirectory, such as `/threads/jan_unixstamp/`, with files like `messages.jsonl` and `thread.json` detailing the thread settings.
