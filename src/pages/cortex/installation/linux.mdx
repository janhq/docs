...
cortex init
```
2. Select between `CPU` or `GPU` mode.
```sh
? Select run mode (Use arrow keys)
> CPU
  GPU
```
3. Cortex will download the required CPU instruction sets if you choose `CPU` mode. If you choose `GPU` mode, Cortex will download the necessary dependencies to use your GPU.
4. Once downloaded, Cortex is ready to use!
</Steps>
## Build from Source
