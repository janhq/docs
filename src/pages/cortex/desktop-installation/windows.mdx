...
> CPU
  GPU
```
3. Select the instruction sets based on your hardware CPU.
```sh
? Select CPU instructions set (Use arrow keys)
> AVX2
  AVX
  AVX-512
```
4. Cortex will download the selected CPU instruction file automatically.
5. Cortex is ready to use!
</Steps>
...
> CPU
  GPU
```
3. Cortex will download the required CPU instruction sets if you choose `CPU` mode. If you opt for `GPU` mode, Cortex will download the necessary dependencies to use your GPU.
4. Cortex is ready to use!
</Steps>
## Build from Source
