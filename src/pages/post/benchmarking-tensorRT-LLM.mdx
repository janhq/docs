...
Jan now supports NVIDIA TensorRT-LLM in addition to llama.cpp, making the backend multi-engine and performant for users with Nvidia GPUs.
...
- **Measurements**:
  ...
  - GPU VRAM utilization metrics from `nvidia-smi` over 14 seconds. _We discovered that `nvidia-smi` on Windows yielded huge differences compared to Windows Task Manager resource monitor for NVIDIA GPUs. However, we decided to use `nvidia-smi`_
  - Throughput (token/sec) using this [Jan snippet](https://github.com/search?q=repo%3Ajanhq%2Fjan%20timeDiffInSeconds&type=code).
...
- Thunderbolt 3 connection (40gbps) for eGPUs
- PCI 4.0 (126.44gbps) for "on device" desktops
...
<Callout type="info" >
  **Pro Tip:** We found an additional **15% increase in performance** with TensorRT-LLM by:
...
Note, this benchmark was not conducted with overclocking, as it is not a default setting for most users. But it's awesome that TensorRT-LLM can go even faster with these tweaks.
</Callout>

Finally, many thanks to the NVIDIA team who helped with this, including Asli Sabanci Demiroz, Annamalai Chockalingam, Jordan Dodge. 

And thanks to Georgi Gerganov for creating llama.cpp and the quick fact check.
